    ...
    def get_users(cls, limit=5):
        query = '''
            select 
                ...
                u0.location,
                u0.signup_date,
                tmp.attribute as phone
            from (select *
                    from ohm_assessment.user order by signup_date DESC limit %d) u0 
                        left outer join (SELECT u2.user_id, u2.attribute, u1.username
                            FROM ohm_assessment.user u1, ohm_assessment.rel_user_multi u2
                            WHERE u2.rel_lookup = 'PHONE') as tmp
                            on u0.user_id = tmp.user_id and u0.username = tmp.username
            order by signup_date desc''' % limit
        ...
        users = list()
        for row in results:
            (user_id, display_name, tier, point_balance, location, signup_date, phone) = row
            if len(users) > 0 and users[-1]['user_id'] == user_id:
                users[-1]['phone'].append(str(phone))
            else:
                if phone:
                    phone = [str(phone)]
                else:
